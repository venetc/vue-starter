 ...
 */
export const remove = <T>(arr: T[], func: (value: T, index: number, array: T[]) => boolean) => {
 ...
 */
export const size = (val: Record<string, unknown> | [] | string) => Array.isArray(val) || typeof val === 'string'
    ? val.length
 ...
 */
export const compareCombine = <T>(...comparators: Comparator<T>[]) => (left: T, right: T) => {
 ...
 * Returns `true` if the provided predicate function returns `true` for all elements in a collection, `false` otherwise.
 */
export const match = <C, T>(...cases: [C, T][]) => {
